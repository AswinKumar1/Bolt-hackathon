  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold text-neutral-800">Student Retention Analytics</h2>
          <p className="text-neutral-600 mt-1">Comprehensive analysis of student retention patterns and key factors</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white rounded-xl border border-neutral-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-neutral-800">{overallStats.totalStudents.toLocaleString()}</p>
            </div>
        ...
        <div className="bg-white rounded-xl border border-neutral-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-neutral-600">Average Retention</p>
              <p className="text-2xl font-bold text-success-600">{overallStats.averageRetention}%</p>
            </div>
        ...
        <div className="bg-white rounded-xl border border-neutral-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-yellow-600">{overallStats.atRiskStudents}</p>
            </div>
        ...
        <div className="bg-white rounded-xl border border-neutral-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-neutral-600">Improved Courses</p>
              <p className="text-2xl font-bold text-primary-600">{overallStats.improvedCourses}</p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Retention Heatmap */}
        <div className="bg-white rounded-xl border border-neutral-200 p-6">
          <h3 className="text-xl font-semibold text-neutral-800 mb-6">Retention Heatmap</h3>
          <RetentionHeatmap />
        </div>

        {/* Key Factors Analysis */}
        <div className="bg-white rounded-xl border border-neutral-200 p-6">
          <h3 className="text-xl font-semibold text-neutral-800 mb-6">Key Retention Factors</h3>
          <div className="space-y-4">
            {keyFactors.map((factor, index) => (
              <div key={index} className="border border-neutral-200 rounded-lg p-4">
                <div className="flex items-start justify-between mb-2">
                  <h4 className="font-medium text-neutral-800">{factor.factor}</h4>
                ...
                <div className="flex items-start space-x-2">
                  ...
                  <p className="text-sm text-success-700">{factor.recommendation}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="bg-white rounded-xl border border-neutral-200 p-6">
        <h3 className="text-xl font-semibold text-neutral-800 mb-6">Course-by-Course Analysis</h3>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-neutral-200">
                ...
                <th className="text-left py-3 px-4 font-medium text-neutral-700">Retained</th>
                <th className="text-left py-3 px-4 font-medium text-neutral-700">Rate</th>
                <th className="text-left py-3 px-4 font-medium text-neutral-700">Trend</th>
                <th className="text-left py-3 px-4 font-medium text-neutral-700">Risk Factors</th>
                <th className="text-left py-3 px-4 font-medium text-neutral-700">Strengths</th>
              </tr>
            </thead>
            <tbody>
              {retentionData.map((course, index) => (
                <tr key={index} className="border-b border-neutral-100 hover:bg-neutral-50">
                  ...
                  <td className="py-4 px-4 text-neutral-600">{course.totalStudents}</td>
                  <td className="py-4 px-4 text-neutral-600">{course.retained}</td>
                  <td className="py-4 px-4">
                    <span className={`font-medium ${
                      course.retentionRate >= 90 ? 'text-success-600' : 
                      course.retentionRate >= 85 ? 'text-yellow-600' : 'text-red-600'
                    }`}>
                    ...
                    <div className="space-y-1">
                      {course.riskFactors.map((risk, riskIndex) => (
                        <div key={riskIndex} className="flex items-center space-x-1">
                          ...
                          <span className="text-xs text-neutral-600">{risk}</span>
                        </div>
                      ))}
                    </div>
                  </td>
                    ...
                    <div className="space-y-1">
                      {course.strengths.map((strength, strengthIndex) => (
                        <div key={strengthIndex} className="flex items-center space-x-1">
                          ...
                          <span className="text-xs text-neutral-600">{strength}</span>
                        </div>
                      ))}
                    </div>
                  </td>
